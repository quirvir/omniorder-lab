...
type CatalogItem = { menuItemId:number; objNum:number; definitionSequence:number; name:string; description:string; price:number; categoryId:string; categoryName:string; imageUrl?:string; imageAlt?:string; modifierGroups:ModifierGroup[] };
...
function normalizeMenu(value:unknown):CatalogItem[] {
  const menu = record(value); const condimentItems = asArray(menu.condimentItems); const groups = asArray(menu.condimentGroups); const familyGroups = asArray(menu.familyGroups); const majorGroups=asArray(menu.majorGroups);
  ...
  const majorById = new Map<number, Record<string,unknown>>(); majorGroups.forEach(item => { const row=record(item); const id=numberOf(row.majorGroupItemId ?? row.majorGroupRef ?? row.id); if(id) majorById.set(id,row); });
  return asArray(menu.menuItems).map(item => {
    ...
    const familyRef=numberOf(row.familyGroupRef); const family=familyById.get(familyRef); const majorRef=numberOf(row.majorGroupRef ?? record(family||{}).majorGroupRef); const major=majorById.get(majorRef); const categorySource=major||family||{}; const categoryId=major?`major-${majorRef}`:family?`family-${familyRef}`:"other"; const categoryName=textOf(record(categorySource).ConsumerContentProperties ? record(record(categorySource).ConsumerContentProperties).consumerName : record(categorySource).consumerName) || textOf(record(categorySource).name) || "Otros"; const image=imageOf(consumer) ?? imageOf(definition) ?? imageOf(family || {});
