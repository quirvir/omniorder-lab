...
export default function Home() {
  ...
  useEffect(() => { const id=sessionStorage.getItem("omniorder.simphonySession"); if(id){ setSession(id); void load(id); } }, []);
  ...
  return <main>
    ...
    <section className="workspace commerce-workspace"><article className="panel order-panel"><div className="panel-heading"><div><p className="eyebrow">{source?"MENÚ SINCRONIZADO":"CATÁLOGO DEMO"}</p><h2>¿Qué te gustaría ordenar?</h2></div><span className="secure-label">{products.length} opciones</span></div><div className="quick-controls"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar en el menú..."/><div className="channel-tabs compact">{(["Web","WhatsApp","Voz"] as Channel[]).map(item=><button key={item} className={channel===item?"active":""} onClick={()=>setChannel(item)}>{item}</button>)}</div></div>
